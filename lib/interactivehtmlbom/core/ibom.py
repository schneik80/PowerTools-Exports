...
wx dependency: `import wx`, the wx settings dialog, the non-CLI Logger branch,
and run_with_dialog() are dropped. The generation path (main -> parse ->
generate_bom -> generate_file) is unchanged. Callers pass their own logger
object exposing info(msg, *args), warn(msg) and error(msg).
"""
...
class Logger(object):
    """Default stdout logger. The Fusion command supplies its own instead."""
...
def open_file(filename):
    ...
    try:
        ...
        log.warn('Failed to open browser: {}'.format(oe))
...
def main(parser, config, logger):
    # type: (EcadParser, Config, Logger) -> str or None
    global log
    log = logger
