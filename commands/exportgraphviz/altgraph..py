...
def command_execute(args: adsk.core.CommandCreatedEventArgs):
    ...
    try:
        ...
        resultString += 'imagepath="./" ; ;\n'  # default is "dot"
        resultString += (
            'node[width=.5; height=.5; fontsize=24; fontname=helvetica; shape=box;];' + '\n'
        )
        ...
        rootclean = re.sub(r"[/\\?%*:|\"<>\x7F\x00-\x1F]", "-", parentOccTrimmed)
        file = os.path.join(temp_dir, f'{rootclean}.png')
        thumbnail = rootComp.createThumbnail(480, 480, 'png')
        ...
        resultString += (
            '"' + parentOccTrimmed + '" [image="./'+re.sub(r"[/\\?%*:|\"<>\x7F\x00-\x1F]", "-", parentOccTrimmed)+'.png", imagepos= "tc", labelloc="b" shape="box", label="'+ parentOccTrimmed +'"] ;\n'
        )

        # Call the recursive function to traverse the assembly and build the output string.
        traverseAssembly1(
            parentOcc, rootComp.occurrences.asList, 1
        )

        resultString += '\n'.join(['"'+item+'" [image="./'+re.sub(r"[/\\?%*:|\"<>\x7F\x00-\x1F]", "-", item)+'.png", imagepos= "tc", labelloc="b" shape="box", label="'+item+'"] ;' for item in resultlist[0:]])
        ...
        writestring = 'dot -Tpdf -O "' + filepath + '"'
...
def traverseAssembly1(parent, occurrences, currentLevel):
    global temp_dir
    for i in range(0, occurrences.count):
        ...
        #foo = f'"{foo2}"[shape=box,image="",label=""] ;\n'
        #inputString += foo
        if foo2 not in resultlist:
            resultlist.append(foo2)

        design = occ.component
        clean = re.sub(r"[/\\?%*:|\"<>\x7F\x00-\x1F]", "-", foo2)
        file = os.path.join(temp_dir, f'{clean}.png')
        thumbnail = design.createThumbnail(480, 480, 'png')
        if not os.path.exists(file):
            try:
                thumbnail.saveToFile(file)
            except:
                futil.log(f"{CMD_NAME} Error Writing Thumbnail {file}")
                next  


        if occ.childOccurrences:
            traverseAssembly1(occ.name, occ.childOccurrences, currentLevel + 1)
